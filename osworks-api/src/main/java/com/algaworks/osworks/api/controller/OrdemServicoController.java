...
public class OrdemServicoController {
	...
	private OrdemServicoModel adicionar(@Valid @RequestBody OrdemServicoInputModel ordemServicoInput) {
		OrdemServico ordemServico = toEntity(ordemServicoInput);
		return toModel(gestaoOrdemServicoService.criar(ordemServico));
	}

	@GetMapping
	private List<OrdemServicoModel> listar() {
		return toCollectionModel(ordemServicoRepository.findAll());
	}

	@GetMapping("/{ordemServicoId}")
	private ResponseEntity<OrdemServicoModel> buscar(@PathVariable Long ordemServicoId) {
